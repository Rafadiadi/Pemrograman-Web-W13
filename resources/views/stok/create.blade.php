...
@section('content')
    ...
    <div class="card" style="background: linear-gradient(135deg, rgba(147, 51, 234, 0.12) 0%, rgba(125, 209, 255, 0.08) 100%); border-left: 4px solid #9333ea;">
        <h2 style="margin:0; color: #fff;">➕ Tambah Stok</h2>
        <p class="muted" style="margin:4px 0 0;">Isi detail stok untuk produk dan toko</p>
    </div>

    <!-- Form -->
    <div class="card" style="margin-top: 16px;">
        <form action="{{ route('stok.store') }}" method="POST" class="grid" style="grid-template-columns: 1fr; gap: 16px;">
            @csrf
            ...
            <div class="form-group">
                ...
                <input type="number" id="jumlah" name="jumlah" value="{{ old('jumlah', 0) }}" min="0" required>
                @error('jumlah')
                    <div class="error">{{ $message }}</div>
                @enderror
            </div>
